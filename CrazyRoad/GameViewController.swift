...
class GameViewController: UIViewController {
   ...
   //     sceneView.showsStatistics = true
    ...
    func setupCamera() {
        ...
        cameraNode.eulerAngles = SCNVector3(x: -toRadians(angle: 60), y: toRadians(angle: 20), z: 0)  // point at floor
    ...
    func setupTraffic() {
        for lane in lanes where lane.trafficNode != nil {
            addActions(for: lane.trafficNode!)
        }
    }
...
//        }
    ...
    func addActions(for trafficNode: TrafficNode) {
        ...
        // Vehicles have different speeds.  Set speed based on type
        driveAction.speed = 1/CGFloat(trafficNode.type + 1) + 0.5
        
        for vehicle in trafficNode.childNodes {
            vehicle.removeAllActions()  // remove old actions before creating vehicls
